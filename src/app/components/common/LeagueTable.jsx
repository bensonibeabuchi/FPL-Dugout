...
const [sortConfig, setSortConfig] = useState({ key: 'rank', order: 'asc' }); 
...
useEffect(() => {
  ...
  const handleCompareTeams = () => {
    ...
    setComparisonData([
      ...
      {
        label: "Captain",
        team1: generalInfo?.elements?.find((player) => player.id === teamDetails1?.picks?.find((p) => p.is_captain)?.element)?.web_name || "-",
        team2: generalInfo?.elements?.find((player) => player.id === teamDetails2?.picks?.find((p) => p.is_captain)?.element)?.web_name || "-",
      },
      {
        label: "Chips Used",
  ...
  const handleToggleTeamPlayers = (teamId) => {
    ...
    <div className="w-full">
      ...
      <div className="flex items-center gap-4 mb-2">
        ...
        {showComparison &&
        <CompareTwoTeams comparisonData={comparisonData} comparisonSquads={comparisonSquads} sortedData={sortedData}
          team1={team1} team2={team2} onClose={handleOnclose} fullTeam={fullTeam} fullLeagueData={fullLeagueData}
          liveGameweek={liveGameweek} generalInfo={generalInfo} /> }
      ...
      <table className="sm:min-w-[320px] w-full border border-gray-300 table-auto">
        <thead>
          <tr className="bg-[#202020] text-white text-[8px] sm:text-base">
            ...
            <th className="p-2 sm:py-10 text-left cursor-pointer max-w-28 items-center mt-3" onClick={()=>
              handleSort('realTotalPoints')}>Total {sortConfig.key === 'realTotalPoints' ? (sortConfig.order === 'asc' ?
              <RiArrowDownSFill color='red' size={20} /> :
              <RiArrowUpSFill color='green' size={20} />) :
              <RiArrowUpSFill color='green' size={20} />}</th>
            <th className="p-2 sm:py-10 text-left cursor-pointer max-w-28 flex items-center mt-3" onClick={()=>
              handleSort('GW')}>GW {sortConfig.key === 'GW' ? (sortConfig.order === 'asc' ? <>
                <RiArrowDownSFill color='red' size={20} />
              </> : <>
                <RiArrowUpSFill color='green' size={20} />
              </>) :
        ...
        <tbody>
          ...
          const viceCaptainName = generalInfo?.elements?.find((player) => player.id === viceCaptainElement)?.web_name ||
          "Unknown";
          ...
          return (
          <>
            <tr key={team.entry} onClick={()=> handleToggleTeamPlayers(team.entry)} className="odd:bg-gray-100 even:bg-white
              text-[8px] sm:text-lg cursor-pointer">
              ...
              <td className='p-2 sm:py-10 text-left max-w-14'>
                <TeamHistory eventRealPoints={eventRealPoints} teamId={team.entry} onRealTotalPointsUpdate={(teamId,
                  realTotalPoints)=> {
              ...
              <td className='p-2 sm:py-10 text-left  max-w-14'>
                {teamDetails?.entry_history?.event_transfers}({teamDetails?.entry_history?.event_transfers_cost ?
                `-${teamDetails.entry_history.event_transfers_cost}` : 0})
              </td>
              <td className='p-2 sm:py-10 text-left max-w-14'>{teamDetails?.entry_history?.total_points - highestPoints}
              </td>
            </tr>

            {isExpanded === team.entry && (
            <tr>
              <td colSpan="9" className="p-2 sm:p-8 m-2">
                <div className="flex flex-wrap sm:gap-8 gap-3 text-xs sm:text-sm font-medium">
                  {teamDetails?.picks?.map(player => {

                  const matchedPlayerPoints = liveGameweek?.elements.find((element) => element.id === player.element);
                  const eventPoints = (matchedPlayerPoints?.stats.total_points ?? 0) * (player.multiplier ?? 1);
                  return (
                  <PlayerCardHorizontal key={player.element} eventPoints={eventPoints}
                    player={generalInfo?.elements?.find(p=> p.id === player.element)} isCaptain={player.is_captain}
